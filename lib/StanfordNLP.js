                ...
                try {
                  ...
                  if (typeof sentences === 'object' && Array.isArray(sentences)) {
                    ...
                    for (_i = 0, _len = _ref4.length; _i < _len; _i++) {
                      sentence = _ref4[_i];
                      if(sentence && sentence.parse) sentence.parsedTree = getParsedTree(sentence.parse);
                    }
                  } else {
                    if(sentences && sentences.parse) sentences.parsedTree = getParsedTree(sentences.parse);
                  }
                } catch (_error) {
                  err = _error;
                  return callback(err);
